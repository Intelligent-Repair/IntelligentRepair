...
**חשוב מאוד:**
**הנחיות חשובות למילוי נתונים:**
1. **רכב חשמלי (EV)**: אם הרכב חשמלי מלא, השאר את שדות השמן, דלק ומידות מיכל ריקים (""). ספק נתונים רק למגבים, צמיגים, נוזל קירור (לסוללה) ומצבר 12V.
2. **מלא כמה שיותר שדות**. אל תחזיר ערך ריק אלא אם כן אין שום מידע זמין לדגם זה בכלל.
3. אם יש מספר גרסאות מנוע/גימור ולא צוין ספציפית - ספק את הנתונים של הגרסה **הנפוצה והסטנדרטית** ביותר לדגם זה בשנתון זה.
4. לחץ אוויר: עדיף לספק טווח תקין (למשל "32-35 PSI") או ערך מומלץ לעומס רגיל, מאשר להשאיר ריק.
5. סוג שמן (לרכבי בנזין/דיזל): ספק את הצמיגות והתקן המומלצים ע"י היצרן (למשל "5W-30 בתקן יצרן").
6. נוזל קירור: ציין את סוג הנוזל (G12, FL22 וכו') וצבע אופייני.
7. מידות צמיגים: ספק את המידה הסטנדרטית הנפוצה.
8. נתונים בעברית (הוראות קצרות) צריכים להיות ברורים ותמשיתיים.
9. השתמש רק בנתונים מאומתים מספר הרכב הרשמי של היצרן
10. אם אינך בטוח ב-100% בערך מסוים - החזר מחרוזת ריקה ""
11. לחץ אוויר צריך להיות ב-PSI בלבד (לדוגמה: "32 PSI")
12. סוג השמן חייב לכלול צמיגות מלאה (לדוגמה: "5W-30 סינתטי מלא")
13. צבע נוזל קירור - ציין את הצבע הספציפי (ירוק, ורוד, כתום, אדום, כחול וכו')
14. סוג נוזל קירור - ציין את הסטנדרט המדויק (G12+, OAT, HOAT, IAT וכו')
15. מידות צמיגים - פורמט סטנדרטי (לדוגמה: "205/55R16")
16. נפח מיכל דלק - בליטרים (לדוגמה: "50 ליטר")
17. מגבים - מידות באינצ'ים (לדוגמה: "24\" + 18\"" או "26\"")
18. מצבר - מפרט מלא כולל קיבולת (לדוגמה: "12V 60Ah 540A")
