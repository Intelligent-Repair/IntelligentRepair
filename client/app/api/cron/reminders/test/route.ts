...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
      console.error("[cron/test] supabase error", error);
    ...
    for (const row of rows) {
      ...
      if (!userEmail) {
        results.push({ id: row.id, type: "test", status: "skipped", reason: "no_email" });
        continue;
      }

      const window = dueInWindow30(row.test_date, row.test_last_sent_at, now);
      if (!row.test_date) {
...
מומלץ לקבוע תור למכון הרישוי ולהתכונן בהתאם.`;
      ...
      try {
        ...
        console.error("[cron/test] send failed", err);
