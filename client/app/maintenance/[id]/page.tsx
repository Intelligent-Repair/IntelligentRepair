    ...
    const loadData = async () => {
      try {
        ...
        if (vehicleData) {
          ...
          // 2. חיפוש ספר רכב תואם (לפי יצרן ודגם)
          const { data: manualData } = await supabase
            .from('manuals')
            .select('*')
            .eq('car_model', vehicleData.model) // הנחה שהשמות תואמים בבסיס הנתונים
            // .eq('year', vehicleData.year) // אפשר להוסיף אם רוצים דיוק לפי שנה
            .single();
